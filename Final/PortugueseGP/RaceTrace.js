Promise.all([
    d3.csv("PortugueseGP/PortGP.csv"), d3.csv("Drivers.csv"), d3.csv("status.csv"), d3.csv("PortugueseGP/PortResults.csv")]).then(([gpData, driverData, statusData, resultsData]) => {

//SVG
    const width = 1200;
    const height = 800;
    const margin = {t: 100, b: 100, l: 50, r: 20};
    const svg = d3.select("#d3-container-RaceTracePort")
        .append("svg")
        .attr("height", height)
        .attr("width", width)
    ...
    svg.append("text")
        ...
        .attr("font-size", "40px")
        .text("Race 3 - 2021 Portuguese Grand Prix");
    ...
    svg.append("rect")
        ...
        .on("mouseover", function () {
            let d = d3.select("#d3-container-RaceTracePort").select("#sc");
            d.attr("stroke", "black")
    ...
    const legend = d3.select("#d3-container-RaceTracePort")
        .append("svg")
            ...
            for (let k = 0; k < rows; k++) {
                ...
                legend.append("rect")
                    ...
                    .on("click", function (elem) {
                        ...
                        let currPath = d3.select("#d3-container-RaceTracePort").select("#P" + d.attr("driverId"));
                        let t = d3.select("#d3-container-RaceTracePort").select("#T" + d.attr("driverId"));
                    ...
                    .on("mouseover", function (elem) {
                        ...
                        const currPath = d3.select("#d3-container-RaceTracePort").select("#P" + d.attr("driverId"));
                        if (d.attr("toggle") === "on") {
                            d3.selectAll(".driverPath")
                                .attr("opacity", ".2");
    ...
    const raceTable = d3.select("#d3-container-RaceTracePort")
        .append("svg")
        .attr("height", rtSet.h)
        .attr("width", rtSet.w)
        ...
        for (let i = 0; i < rows; i++) {
            raceTable.append("rect")
                ...
                .on("click", function (elem) {
                    let d = d3.select(this);
                    let currPath = d3.select("#d3-container-RaceTracePort").select("#P" + d.attr("driverId"));
                    let l = d3.select("#d3-container-RaceTracePort").select("#L" + d.attr("driverId"));
